...
final class CropWindowHandler {
  ...
  private final Polygon mPolygon = new Polygon();
  ...
  private final Polygon mGetPolygon = new Polygon();
  ...
  private CropWindowMoveHandler.Type getPolygonPressedMoveType(float x, float y, float targetRadius)
  {
    ...
    else if (CropWindowHandler.isInHorizontalTargetZone(x, y, mPolygon.topLeftX, mPolygon.topRightX, mPolygon.topLeftY, targetRadius))
    {
      moveType = CropWindowMoveHandler.Type.TOP;
    }
    else if (CropWindowHandler.isInHorizontalTargetZone(x, y, mPolygon.bottomLeftX, mPolygon.bottomRightX, mPolygon.bottomLeftY, targetRadius))
    {
      moveType = CropWindowMoveHandler.Type.BOTTOM;
    }
    else if (CropWindowHandler.isInVerticalTargetZone(x, y, mPolygon.topLeftX, mPolygon.topLeftY, mPolygon.bottomLeftY, targetRadius))
    {
      moveType = CropWindowMoveHandler.Type.LEFT;
    }
    else if (CropWindowHandler.isInVerticalTargetZone(x, y, mPolygon.topRightX, mPolygon.topRightY, mPolygon.bottomRightY, targetRadius))
    {
      moveType = CropWindowMoveHandler.Type.RIGHT;
    }
    else if (CropWindowHandler.isInCenterTargetZone(x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom)
            && !focusCenter())
    {
      moveType = CropWindowMoveHandler.Type.CENTER;
    }
  ...
  private static boolean isInCenterTargetZone(float x, float y, float topLeftX, float topLeftY, float topRightX, float topRightY, float bottomLeftX, float bottomLeftY, float bottomRightX, float bottomRightY)
  {
    return x > topLeftX && x < topRightX && x > bottomLeftX && x< bottomRightX
            && y > topLeftY && y < bottomLeftY && y > topRightY && y < bottomRightY;
  }
